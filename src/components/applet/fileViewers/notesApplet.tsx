...
export const NotesApplet: React.FC<IProps> = (props: IProps) => {
    ...
    useEffect(() => {
        ...
        if (file?.toLowerCase?.().includes(MarkdownFile.secrets?.toLowerCase?.())) {
            ctx.virtualAssistantService.say?.(translate(LocaleKey.clippySecretFound));
            ctx.virtualAssistantService.play?.(virtualAssistantAnimations.writing);
            addSecretIfNotFound({
    ...
    const getContentFromDataService = async (index: number) => {
        ...
        const netResult: ResultWithValue<string> = await ctx.dataService.getMarkdownFile<string>(urlToFetch);
